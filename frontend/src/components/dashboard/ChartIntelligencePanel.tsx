...
export function ChartIntelligencePanel({ data }: { data: LiveMarketData | null }) {
    ...
    const signal = chartAnalysis?.bias ?? 'WAIT';
    const confidence = chartAnalysis?.confidence ?? 0;
    const wave = chartAnalysis?.wave ?? '?';
    const wavePattern = chartAnalysis?.wave_pattern ?? '—';
    const neoPattern = chartAnalysis?.neo_wave_pattern;
    const trend = chartAnalysis?.regime ?? 'RANGE';
    const spot = chartAnalysis?.price ?? 0;
    const bos = chartAnalysis?.bos_detected;
    const mss = chartAnalysis?.mss_detected;
