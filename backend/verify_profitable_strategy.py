#!/usr/bin/env python3
"""
Final Verification - StrikeIQ Profitable Strategy Engine
"""
...
def test_profitable_strategy_engine():
    ...
    # Test 4: Trend Strategy
    print("\n🔧 Test 4: Trend Strategy...")
    try:
        from ai.strategy_engine import generate_trade
        
        # Mock time filter to always return True for testing
        ...
        # Create analytics dict directly
        analytics = {
            "pcr": 0.8,  # Low PCR
            "rsi": 60,   # High RSI
            "volatility": 0.04,
            "liquidity": 150000  # High liquidity for momentum filter
        }
    ...
    try:
        ...
        # Create range-bound analytics
        range_analytics = {
            "pcr": 0.6,  # Very low PCR
            "rsi": 50,   # Neutral RSI
            "volatility": 0.02,  # Low volatility
            "liquidity": 150000  # High liquidity
        }
        
        signal, strategy = generate_trade(snapshot, range_analytics)
        
        if signal == "BUY_PUT" and strategy == "REVERSAL":
            print("✅ Range strategy generates correct signal")
            results.append(True)
        else:
            print(f"❌ Expected BUY_PUT/REVERSAL, got {signal}/{strategy}")
            results.append(False)
    ...
    try:
        ...
        if signal == "NONE" and strategy == "INVALID":
            print("✅ Invalid data filtered correctly")
            results.append(True)
        else:
            print(f"❌ Invalid data not filtered: {signal}/{strategy}")
            results.append(False)
        
        # Test no edge conditions
        neutral_analytics = {
            "pcr": 1.0,  # Neutral PCR
            "rsi": 50,   # Neutral RSI
            "volatility": 0.03,
        ...
        signal, strategy = generate_trade(snapshot, neutral_analytics)
        
        if signal == "NONE" and strategy == "NO_EDGE":
            print("✅ No edge conditions filtered correctly")
            results.append(True)
        else:
            print(f"❌ No edge not filtered: {signal}/{strategy}")
            results.append(False)
