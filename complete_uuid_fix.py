...
print("=== 🔧 STEP 1 — ADD UUID COLUMN TO ai_signal_logs ===")
try:
    cur.execute("""
    ...
    print(f"⚠️  id_uuid might already exist: {e}")
...
try:
    ...
    print(f"⚠️  signal_id_uuid might already exist: {e}")

print("\n=== 🔧 STEP 3 — DROP DEPENDENT CONSTRAINT ===")
try:
    cur.execute("""
        ALTER TABLE signal_outcomes
        DROP CONSTRAINT signal_outcomes_signal_id_fkey;
    """)
    conn.commit()
...
try:
    cur.execute("ALTER TABLE ai_signal_logs DROP COLUMN id CASCADE;")
    conn.commit()
    print("✅ ai_signal_logs.id dropped")
    
    cur.execute("ALTER TABLE signal_outcomes DROP COLUMN signal_id;")
    conn.commit()
...
try:
    ...
    print(f"❌ Failed to rename columns: {e}")
    conn.rollback()
    raise
...
print("\n=== 🔧 STEP 7 — ADD PRIMARY KEY TO ai_signal_logs ===")
try:
    cur.execute("""
        ALTER TABLE ai_signal_logs
        ADD PRIMARY KEY (id);
    ...
    print(f"⚠️  Primary key might already exist: {e}")
...
try:
    ...
    print(f"❌ Failed to add fk_prediction: {e}")
    conn.rollback()
    raise
...
try:
    ...
    print(f"❌ Failed to add signal_outcomes FK: {e}")
    conn.rollback()
    raise
